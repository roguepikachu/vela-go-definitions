...
func ServiceAccount() *defkit.TraitDefinition {
	...
	// Parameters
	name := defkit.String("name").Description("Specify the name of ServiceAccount")
	...
	privilegesHelper := defkit.Struct("Privileges").WithFields(
		defkit.Field("verbs", defkit.ParamTypeArray).Of(defkit.ParamTypeString).Description("Specify the verbs to be allowed for the resource"),
